/**
 * Audit variant model_key pro Octavii (Skoda): všechny varianty, count a ukázkové titulky.
 * Pomáhá před sjednocením aliasů zjistit, které varianty v DB jsou a co do nich padá.
 *
 * Usage:
 *   npx tsx lib/maintenance/auditOctaviaModelKeyVariants.ts
 *   npx tsx lib/maintenance/auditOctaviaModelKeyVariants.ts --source=sauto
 */
...
export type AuditOctaviaVariantsResult = {
  source: string;
  total_rows: number;
...
export async function auditOctaviaModelKeyVariants(
  supabase: SupabaseClient,
  source: string
): Promise<AuditOctaviaVariantsResult> {
  const brandNorm = "skoda";
  const modelSlug = "octavia";
  const pattern = "%octavia%";
  ...
  const filtered = merged.filter((r) => {
    ...
    return mk.includes(modelSlug) || raw.includes("octavia");
  });
...
function printResult(r: AuditOctaviaVariantsResult): void {
  console.log("\n--- Octavia model_key variants (Skoda, source=" + r.source + ") ---");
...
async function main(): Promise<void> {
  ...
  const result = await auditOctaviaModelKeyVariants(supabase, source);
  printResult(result);
}
